...
function Invoke-VMProcess {
  ...
  if ($vm.PowerState -eq 'PoweredOn') {
    try {
      ...
      $TasksCompleted += '☑ Shutdown (graceful)'
    } catch {
      $TasksCompleted += '✗ Shutdown (graceful)'
    }
    $timeout = 60
    ...
    if ($vm.PowerState -ne 'PoweredOff') {
      try {
        ...
        $TasksCompleted += '☑ Shutdown (forced)'
      } catch {
        $TasksCompleted += '✗ Shutdown (forced)'
      }
    }
  } else { $AlreadyPoweredOff = $true; $TasksCompleted += '☑ Already powered off' }
  # NIC disconnect
  try {
    ...
    $TasksCompleted += '☑ NIC Disconnected'
  } catch {
    $TasksCompleted += '✗ NIC Disconnected'
  }
  # Move to _DECOM
  try {
    ...
    $TasksCompleted += '☑ Moved to _DECOM'
  } catch {
    $TasksCompleted += '✗ Moved to _DECOM'
  }
  # Rename
